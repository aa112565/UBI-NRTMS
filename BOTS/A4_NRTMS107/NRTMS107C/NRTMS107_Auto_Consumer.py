...
logger = logging.getLogger("NRTMS107C")
logger.setLevel(logging.INFO)
handler = RotatingFileHandler('./logs/NRTMS107C.log', maxBytes=50*1024*1024, backupCount=10)
...
def to_wf(rec,amt_thres):
    ...
    with engine.connect() as conn:
        ...
        add_info = 'ACCOUNT NO~' + rec['foracid'] + '^ACCOUNT NAME~' + rec['acct_name'] + '^A/C OPEN DATE~' + rec['acct_opn_date'] + '^TRANSACTION DATE~' + rec['tran_date'] + '^TRANSACTION AMOUNT~' + rec['tran_amt'] + '^TRANSACTION ID~' + rec['tran_id'] + '^SCHME CODE~' + rec['schm_code'] + '^TRANSACTION TYPE~' + rec['tran_type'] + '^PART TRANSACTION TYPE~' + rec['part_tran_type'] + '^TRANSACTION PARTICULARS~' + rec['tran_particular'] + '^CUSTOMER TYPE~' + rec['cust_type'] + '^CUSTOMER HEALTH~' + (rec['cust_hlth'] if rec['cust_hlth'] is not None else '')
        reclst = [alertNo,alertinfo['v_type'],ewiid, alertinfo['v_desc'], alertinfo['v_severity'], alertinfo['v_source'], freq, rec['cif_id'], rec['acct_name'],rec['sol_id'], branchname['v_branch_name'], msg,datetime.strptime(rec['xason'],'%Y-%m-%d %H:%M:%S'),alertinfo['v_display_area'], alertinfo['v_significance'], alertinfo['v_risk_mitigation'], alertnum,rundate,close_by_date,branchname['br_code'],branchname['region_code'],branchname['region_name'],branchname['zone_code'],branchname['zone_name'],rec['foracid'],datetime.strptime(rec['txn_dtm'],'%Y-%m-%d %H:%M:%S'),rec['tran_amt'],add_info]    
...
def sendmail(alertnum,ewiid,alertmsg,solid,branchname,custname,alertdesc,alertrefid,rundate,alertNo):
    try:
        ...
        <p><h4><b>In case of any irregularity,appropriate corrective action should be taken immediately.</b></h4></p></br>
