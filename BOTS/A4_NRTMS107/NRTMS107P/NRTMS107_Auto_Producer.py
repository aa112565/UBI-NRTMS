...
logger = logging.getLogger("NRTMS107P")
logger.setLevel(logging.INFO)
handler = RotatingFileHandler('./logs/NRTMS107P.log', maxBytes=50*1024*1024, backupCount=10)
...
def data_extraction_process(start_str):
    ...
    qry = '''   SELECT /*+parallel(8)*/ SUBSTR(division_name,8,30) Zone_Name,
                  ...
                  to_char(dtd.tran_amt) tran_amt,
                   ...
                   TRAN_PARTICULAR,
              cust_type,
             (select REF_DESC from tbaadm.rct where REF_REC_TYPE='43' and REF_CODE=cust_type and rct.DEL_FLG='N' and rownum<2) cust_type_desc,
             crmuser.accounts.cust_hlth,
             (select REF_DESC from tbaadm.rct where REF_REC_TYPE='17' and REF_CODE=crmuser.accounts.cust_hlth and rct.DEL_FLG='N' and rownum<2) riskdesc,
            ...
            AND   dtd.tran_amt >=:alertThreshold
            AND   dtd.del_flg = 'N'
            AND   dtd.pstd_date>TO_DATE(:dateFilter,'dd-mm-yy HH24:MI:SS') ORDER BY TO_CHAR(dtd.pstd_date,'dd-mm-yy HH24:MI:SS')'''
                ...
                with nrtmsDB.connect() as nrtmsDBCon:    
                    with nrtmsDBCon.begin():
                        if rslttemp:
                            ...
                            audit_sno = nrtmsDBCon.execute("SELECT NRTMS_107_AUDIT_SNO_SEQUENCE.NEXTVAL FROM dual").fetchone()[0]
                            ...
                            sql = "INSERT INTO NRTMS_107_AUDIT_LOGS(SNO,DATA_EXTRACTION_START_TIME,DATA_EXTRACTION_END_TIME, total_TIME_taken,NO_OF_RECORDS,LOG_STATUS) VALUES (:SNO,:DATA_EXTRACTION_START_TIME, :DATA_EXTRACTION_END_TIME, :total_TIME_taken,:NO_OF_RECORDS,:LOG_STATUS)"
                            nrtmsDBCon.execute(sql,data)
                            #logger.info(datetime.strftime(start_str,'%Y-%m-%d %H:%M:%S')+'<<'+'>> Count Rsltemp: '+str(len(rslttemp)))
                           #logger.info(start_str+' Count Rsltemp: '+str(len(rslttemp)))
                            for r in rslttemp:  
                                logger.info(r)
                                xason = datetime.now()
                                nrtmsDBCon.execute('insert into NRTMS107(ZONE_NAME,REGION_NAME, SOL_ID,SOL_DESC, BR_CODE,SOLCAT,CIF_ID,FORACID, ACCT_NAME,ACCT_OPN_DATE,TRAN_DATE,TRAN_AMT,TRAN_ID,SCHM_CODE,TRAN_TYPE,PART_TRAN_TYPE,TRAN_PARTICULAR,CUST_TYPE,CUST_TYPE_DESC,CUST_HLTH,RISKDESC,TXN_DTM,ason) values(:p1,:p2,:p3,:p4,:p5,:p6,:p7,:p8,:p9,:p10,:p11,:p12,:p13,:p14,:p15,:p16,:p17,:p18,:p19,:p20,:p21,:p22,:p23)', [r[0],r[1],r[2],r[3],r[4],r[5],r[6],r[7],r[8],r[9],r[10],r[11],r[12],r[13],r[14],r[15],r[16],r[17],r[18],r[19],r[20],r[21],xason])
                                ##
                                rec = dict(r) 
