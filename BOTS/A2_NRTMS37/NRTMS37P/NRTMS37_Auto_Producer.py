...
logger = logging.getLogger("NRTMS37P")
logger.setLevel(logging.INFO)
handler = RotatingFileHandler('./logs/NRTMS37P.log', maxBytes=50*1024*1024, backupCount=10)
...
def data_extraction_process(start_str):
    ...
    AND   dtd.del_flg = 'N'
    AND   gam.del_flg = 'N' order by  main_acct.tran_date, TO_CHAR(main_acct.txn_dtm,'DD-MM-YYYY HH24:MI:SS')'''
                ...
                with nrtmsDB.connect() as nrtmsDBCon:    
                    with nrtmsDBCon.begin():
                        if rslttemp:
                            ...
                            audit_sno = nrtmsDBCon.execute("SELECT NRTMS_37_AUDIT_SNO_SEQUENCE.NEXTVAL FROM dual").fetchone()[0]
                            ...
                            sql = "INSERT INTO NRTMS_37_AUDIT_LOGS(SNO,DATA_EXTRACTION_START_TIME,DATA_EXTRACTION_END_TIME, total_TIME_taken,NO_OF_RECORDS,LOG_STATUS) VALUES (:SNO,:DATA_EXTRACTION_START_TIME, :DATA_EXTRACTION_END_TIME, :total_TIME_taken,:NO_OF_RECORDS,:LOG_STATUS)"
                            nrtmsDBCon.execute(sql,data)
                            #logger.info(datetime.strftime(start_str,'%Y-%m-%d %H:%M:%S')+'<<'+'>> Count Rsltemp: '+str(len(rslttemp)))
                           #logger.info(start_str+' Count Rsltemp: '+str(len(rslttemp)))
                            for r in rslttemp:  
                                logger.info(r)
                                xason = datetime.now()
                                nrtmsDBCon.execute('insert into NRTMS37(SOL,SOL_ID,CIF_ID,FORACID,ACCT_NAME,CR_ACCT_NO,CR_ACCT_NAME,TRAN_TYPE,TRAN_ID,TRAN_AMT,TRAN_DATE,TXN_DTM,ASON) values(:p1,:p2,:p3,:p4,:p5,:p6,:p7,:p8,:p9,:p10,:p11,:p12,:p13)', [r[0],r[1],r[2],r[3],r[4],r[5],r[6],r[7],r[8],r[9],r[10],r[11],xason])
                                ##
                                rec = dict(r) 
                                ...
                                if datetime.strptime(last_tran_time,'%d-%m-%Y %H:%M:%S')>datetime.strptime('01-05-2023 00:00:00','%d-%m-%Y %H:%M:%S'):
