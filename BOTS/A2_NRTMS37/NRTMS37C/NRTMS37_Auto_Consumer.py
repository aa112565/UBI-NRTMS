...
logger = logging.getLogger("NRTMS37C")
logger.setLevel(logging.INFO)
handler = RotatingFileHandler('./logs/NRTMS37C.log', maxBytes=50*1024*1024, backupCount=10)
...
def sendmail(alertnum,ewiid,alertmsg,solid,branchname,custname,alertdesc,alertrefid,rundate,alertNo):
    try:
        ...
        tocc=''
        wfdate=rundate.strftime("%m.%d.%Y %H:%M:%S")
        with engine.connect() as conn:
