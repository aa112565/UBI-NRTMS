...
def to_wf(rec,amt_thres):
    ...
    msg='It is observed that the branch has debited Rs. '+rec['tran_amt']+' to A/c No. '+rec['foracid']+' on '+rec['tran_date']+' towards the shortage in cash, which is highly irregular. Please inform us, whether the branch has obtained permission from your office for such type of debits. If so, please inform us that the action has been initiated against the earring staff/officials.'
    #msg = 'On '+rec['txn_dtm']+', Debit transaction in Suspence Account Number: '+rec['foracid']+' of Rs.'+rec['tran_amt']+' with transaction ID:'+rec['tran_id']+' have taken place beyond the threshold of Rs.'+amt_thres    
    with engine.connect() as conn:
        alertrefid = conn.execute('select mst_wf_alert_ref_id.nextval from dual').fetchone()[0]
        rundate = datetime.now()
        reclst = [alertrefid,alertinfo['v_type'],ewiid, alertinfo['v_desc'], alertinfo['v_severity'], alertinfo['v_source'], freq, rec['cif_id'], rec['acct_name'],rec['sol_id'], branchname['v_branch_name'], msg,datetime.strptime(rec['xason'],'%Y-%m-%d %H:%M:%S'),alertinfo['v_display_area'], alertinfo['v_significance'], alertinfo['v_risk_mitigation'], alertnum,rundate,close_by_date,branchname['v_location_code'],branchname['v_region_code'],branchname['v_region_name'],branchname['v_zone_code'],branchname['v_zone_name'],rec['foracid'],datetime.strptime(rec['txn_dtm'],'%Y-%m-%d %H:%M:%S'),rec['tran_amt']]    
    ...
    print('To WF:', rec['foracid'], rec['foracid'])
    logging.info('To WF: '+rec['foracid'])
